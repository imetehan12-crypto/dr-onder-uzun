/* ============================================================
   DR. ÖNDER UZUN — scroll engine
   Canvas frame-sequence scrub sections + scroll reveals + counters
   (scroll-cinematic skill engine, site-customized)
   ============================================================ */
function initScrub(cfg) {
  ...
  function update() {
    ...
    if (idx !== current) {
      ...
      if (readout) readout.textContent = "KARE " + String(idx + 1).padStart(3, "0");
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  lenis.on("scroll", ({ scroll }) => {
    ...
    if (nav) nav.classList.toggle("scrolled", scroll > 80);
  });
});
